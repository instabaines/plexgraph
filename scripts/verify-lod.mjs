...
try {
  ...
  await page.goto(url);
  const lod = () => page.evaluate(() => window.__hyperloomHandle.getLodState());
  await page.waitForFunction(() => window.__hyperloomHandle?.getVisibleNodeCount?.() === 6000);
  await page.waitForTimeout(3000);
  await page.evaluate(() => window.__hyperloomHandle.fitView()); await page.waitForTimeout(700);
  ...
  const svg0 = await page.evaluate(() => window.__hyperloomHandle.exportSVG());
  ...
  await page.evaluate(() => window.__hyperloomHandle.zoomBy(40)); await page.waitForTimeout(900);
  state = await lod();
  assert.equal(state.mode, 'region'); assert.ok(state.nodesDrawn > 0 && state.nodesDrawn <= 5000);
  assert.equal(await page.evaluate(() => window.__hyperloomHandle.getVisibleNodeCount()), 6000, 'the whole graph is still the base set');
  await page.evaluate(() => window.__hyperloomHandle.fitView()); await page.waitForTimeout(900);
  ...
  const svg1 = await page.evaluate(() => window.__hyperloomHandle.exportSVG());
  ...
  await page.waitForFunction(() => window.__hyperloomHandle.getVisibleNodeCount() === 2400 || window.__hyperloomHandle.getVisibleNodeCount() === 3600);
  const shown = await page.evaluate(() => window.__hyperloomHandle.getVisibleNodeCount());
